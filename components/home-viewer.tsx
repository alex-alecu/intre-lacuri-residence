...
import {ArrowUpRight, Blocks, ChevronRight, Compass, DoorOpen, FileText, Footprints, House, Layers3, Maximize, Minimize, Moon, Pause, RotateCcw, Sun, X} from 'lucide-react';
...
export default function Home() {
  ...
  return <main className={`home-app ${mode==='walk'?'walk-mode':''} ${touch?'touch-device':''} ${expanded?'expanded':''}`}>
    ...
    <div className="layout-bar">
      ...
      <span className="layout-detail" aria-live="polite">{layout==='social'?'Insulă · Dining separat · TV 160 cm':layout==='suite'?'Perete nou de 15 cm · Dressing':'Amenajarea inițială'}</span>
    </div>
    <div className="workspace">
      <aside className="room-panel">
        ...
        <div className="area-stats"><div><strong>{layout==='social'?'2':'3'}</strong><span>Dormitoare</span></div><div><strong>{layout!=='original'?'1':'2'}</strong><span>{layout!=='original'?'Birou':'Birouri'}</span></div><div><strong>{layout!=='original'?'1':'6'}</strong><span>{layout!=='original'?'Dressing':'Locuri la masă'}</span></div></div>
      ...
      <section className="model-area" aria-label="Model interactiv al locuinței">
        ...
        <div className="view-bottom"><div className="mini-map"><span>EȘTI AICI</span><svg viewBox="-1 -1 16.2 12.1" aria-label="Harta locuinței">{rooms.filter(r=>r.id!=='connection').map(r=><polygon key={r.id} points={(roomFootprints[r.id]??[[r.x,r.z],[r.x+r.w,r.z],[r.x+r.w,r.z+r.d],[r.x,r.z+r.d]]).map(p=>p.join(',')).join(' ')} fill={r.id===selected?'#bd9d75':'#e2ded4'} stroke="#fff" strokeWidth=".1"/>)}<circle cx={position.x} cy={position.z} r=".26" fill="#3f5847" stroke="white" strokeWidth=".12"/></svg></div></div>
    ...
    <Sheet open={notes} onOpenChange={setNotes}><SheetContent className="source-sheet" showCloseButton={false}><SheetClose className="notes-close" aria-label="Închide"><X size={20}/></SheetClose><SheetTitle>Despre amenajare</SheetTitle><SheetDescription>O locuință gândită pentru întreaga familie.</SheetDescription><div className="source-body">
      ...
      {layout!=='original'?<>
        <h3>Dormitor matrimonial și dressing</h3><p>Fostul living devine dormitor, cu pat de 2 × 2 m, tăblie tapițată, două noptiere și dulapuri suplimentare în locul televizorului. Peretele nou de 15 cm are o întoarcere scurtă spre camera copilului. Ușa de 1 m se deschide în dormitor. Holul comun păstrează accesul la baie și la celelalte camere.</p>
        <h3>Un dressing în locul biroului</h3><p>Zona de dressing se extinde din fostul birou spre dormitor. Două corpuri din nuc, de 2,20 m și 1,60 m, au o adâncime de 60 cm. Ușile glisante închid toate compartimentele. Oglinda înaltă și taburetul tapițat completează spațiul. Fereastra rămâne liberă.</p>
        <h3>Spațiu pentru trecere</h3><p>Planul propus păstrează aproximativ 81 cm pe latura de est a patului și 75 cm între pat și dulapurile de 36 cm adâncime. Acestea au compartimente închise pentru haine împăturite; barele pentru umerașe rămân în dressing. Holul din dreptul băii are aproximativ 1,08 m. Suprafața închisă este de aproximativ 20,94 m², inclusiv zona de dressing de 5,69 m². Valorile sunt calculate din model și trebuie verificate la fața locului.</p>
      ...
      {layout==='original'?<><h3>Camera fetiței</h3><p>Patul-căsuță are un cadru din lemn și lumină caldă. Norii luminoși, dulapul închis pentru cărți și jucării și cortul de lectură completează spațiul de joacă. Textilele crem și roz pudrat păstrează atmosfera calmă.</p></>:<><h3>Camera copilului</h3><p>Un pat tapițat de 90 × 190 cm rămâne pe latura de est. Locul de joacă are o saltea moale, un cadru jos de cățărat și un tobogan din lemn. Dulapul de 2,75 m are uși glisante. Dulapul închis pentru jucării, corpul suspendat, sertarul de sub pat și scaunul de lectură completează camera. Ușa de nord se deschide spre balcon.</p><h3>Gaming și birou</h3><p>Biroul de 1,60 m este pe peretele de vest și are trei sertare. Canapeaua de 2,40 m este orientată spre televizor. Ecranul are diagonala de 180 cm și formatul 16:9. Dulapul cu uși glisante ocupă tot peretele de est, lung de 3 m. Corpurile închise deasupra biroului și măsuța cu depozitare adaugă spațiu pentru obiecte. Corpul de sub televizor păstrează consola și accesoriile.</p></>}
      ...
      <h3>Uși și băi</h3><p>Ușa {layout==='original'?'camerei de oaspeți':'camerei copilului'} este pe peretele dinspre hol, aproape de deschiderea din planul etajului. Golul nedorit din peretele băii a fost închis. Căzile, lavoarele și vasele WC sunt orientate conform planurilor.</p>
