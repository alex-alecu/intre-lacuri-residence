...
assert.deepEqual(home.fixtures.find(f=>f.name==='West WC'),{name:'West WC',x:7.58,z:3.52,w:.47,d:.70,rotation:0});
assert.deepEqual(home.fixtures.find(f=>f.name==='East WC'),{name:'East WC',x:8.62,z:1.20,w:.47,d:.70,rotation:Math.PI/2});
...
const targets=[...rooms.map(r=>({id:r.id,point:r.visit})),{id:'west-balcony',point:[1.9,-.28]},{id:'west-balcony-arm',point:[-1.1,3.3]},{id:'east-balcony',point:[12.2,-1.0]},{id:'east-balcony-arm',point:[15.4,2.7]},{id:'kitchen-preparation',point:[11.3,4.95]},{id:'dining-south',point:[11.35,7.75]},{id:'living-desk-approach',point:[1.24,8.05]},{id:'guest-desk-approach',point:[1.55,3.6]}];
...
const report={playerRadius:PLAYER_RADIUS,gridStep:step,meshes,triangles,obstacleCount:home.obstacles.length,reachableGridPoints:queue.length,rooms:results,fixtures:home.fixtures,balconyAreas:balconies.map(b=>({name:b.name,area:polygonArea(b.points)}))};
