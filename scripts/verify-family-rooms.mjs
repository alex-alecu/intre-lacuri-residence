...
export function verifyFamilyRooms(home, canOccupy) {
  ...
  assert.ok(desk.min.x >= 4 && desk.max.x <= 8 && desk.max.z <= 3, 'The desk is in the former child room');
  assert.equal(home.obstacles.filter(o => o.name === 'Office desk' && o.x < 4).length, 0, 'The corner bedroom has no desk');
  ...
  assert.ok(sofa.max.x-sofa.min.x>=2.399,'The office sofa is at least 2.40 m long');
  ...
  const table=bounds('Office storage table');
  const tableDoors=home.furniture.getObjectByName('Office storage table').children.filter(o=>o.name==='Closed storage front');
  for(const door of tableDoors){
    door.geometry.computeBoundingBox();
    assert.ok(door.geometry.boundingBox.getSize(new T.Vector3()).x<media.min.z-table.max.z,'The storage table doors can open before the TV cabinet');
  }
  ...
  const screenBounds=new T.Box3().setFromObject(screen,true).getSize(new T.Vector3());
  ...
  assert.ok(media.min.z-sofa.max.z>=1.40,'The sofa and TV cabinet have at least 140 cm of clear floor');
